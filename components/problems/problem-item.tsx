      ...
      <Item className={className} variant={"default"} size={"xs"}>
        <ItemContent className="flex-row justify-between items-center">
          ...
          <div className="flex flex-1">
            <ItemDescription className="text-sm flex w-full items-center justify-end gap-2">
              {showProgress ? (
                <Badge
                  variant="outline"
                  className={
                    isSolved ? "border-emerald-500/40 text-emerald-600" : "border-muted-foreground/40 text-muted-foreground"
                  }
                >
                  {isSolved ? "Done" : "Not yet"}
                </Badge>
              ) : null}

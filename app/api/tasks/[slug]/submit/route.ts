...
const pollJudge0Result = async (
  ...
  for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt += 1) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 && body.includes("base64_encoded=true")) {
        responseBase64 = true;
        continue;
      }
      throw new Error(
...
const pickFinalStatus = (
  statuses: Array<ReturnType<typeof mapJudge0Status>>,
) => {
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  if (
    ...
    !allowedLanguageIds.includes(languageId)
  ) {
    return NextResponse.json(
  ...
  const judgeBaseUrl = normalizeJudgeBaseUrl(process.env.JUDGE0_BASE_URL || "");
